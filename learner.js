...
class LearningSystem {
  constructor(learningData, confidenceData) {
    ...
    this.minConfidence = 50; // Minimum confidence floor
    this.maxConfidence = 95; // Maximum confidence ceiling
  }

  // Calculate confidence score based on multiple factors
  calculateConfidence(strength, historicalAccuracy, confirmationScore) {
    ...
    const confidence = 
      ...
      (confirmationScore * confirmWeight);

    return Math.max(this.minConfidence, Math.min(this.maxConfidence, Math.round(confidence)));
  }
  ...
  updateHistory(newSignals) {
    ...
    if (this.data.history.length > 2000) {
      this.data.history = this.data.history.slice(-2000);
    }
